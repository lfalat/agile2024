...
namespace AGILE2024_BE.Controllers
{
    ...
    public class RegisterController : ControllerBase
    {
        ...
        {
            //return Ok(_config["JWT_Secret"]); TODO: Prerobiť JWS Token
        ...
        public IActionResult Post([FromBody] RegisterRequestCustom req)
        {
            ...
            User user = new User()
            {
                ...
                Name = req.Name,
                Surname = req.Surname,
                Title_after = req.Title_After,
                Title_before = req.Title_Before,
                Password = hashedPassword,
                Salt = salt,
                Role = _context.Roles.Where(r => r.Id_role == req.Id_Role).FirstOrDefault()
            };
            _context.Add(user);
            _context.SaveChanges();
            return Ok("Vytvoril sa nová používateľ");
        }
        ...
        private IActionResult? CheckForm([FromBody] RegisterRequestCustom req)
        {
            ...
            var role = _context.Roles.Where(r => r.Id_role == req.Id_Role).FirstOrDefault();
            if (role == null)
            {
                return BadRequest("Rola neexistuje");
            }
            if (returnAction != null)
                return returnAction;
            return null;
    ...
    public class RegisterController : ControllerBase
{
    ...
    public IActionResult Post([FromBody] RegisterRequestCustom req)
    {
        ...
        using var connection = _database.OpenConnection();
        string query2 = "insert into user_tab (email, password, salt, name, surname, id_role, title_before, title_after) values(@parEmail, @parPassword, @parSalt, @parName, @parSurname, @parRole, @parTitle_Before, @parTitle_After)";
